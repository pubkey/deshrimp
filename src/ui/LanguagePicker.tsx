 ...
 * A control the size of the theme toggle, sitting immediately to its right,
 * showing the current language as its flag, and the language's own name beside
 * it past 900px. `<Page>` renders it on every page; a page never places it
 * itself.
 *
 ...
 * ## Examples
 * ```tsx
 * <LanguagePicker value="de" />                          // label, not a picker
 * <LanguagePicker value={lang} languages={['de', 'en']}   // picker
 *     onChange={(l) => save({ lang: l })} />
 * ```
 *
 * ## Changelog
 * - 2026-09-15 Drawn flags, and the language's name beside it on a wide
 *   screen. His calls, both.
 ...
 */
const LABEL: Record<UiLang, { full: string }> = {
    de: { full: 'Deutsch' },
    en: { full: 'English' },
    es: { full: 'Español' },
    fr: { full: 'Français' },
...
export function LanguagePicker({ value, languages, onChange, className }: LanguagePickerProps) {
    ...
    if (!pickable) {
        return (
            <span
                className={cx('ui-iconbtn', 'ui-iconbtn-wide', 'ui-langtag', className)}
                title={shown.full}
                aria-label={shown.full}
            >
                <Flag lang={current} />
                <span className="ui-iconbtn-label">{shown.full}</span>
            </span>
        );
    }

    return (
        <span className={cx('ui-iconbtn', 'ui-iconbtn-wide', 'ui-langpick', className)}
            title={shown.full}>
            <Flag lang={current} />
            <span className="ui-iconbtn-label">{shown.full}</span>
            <select
